...
@router.post("/payouts")
async def request_payout(request: Request, body: PayoutRequest):
    p = _require(request)
    if body.amount <= 0: raise HTTPException(400, "Amount must be positive")
    with get_db() as conn:
        user = conn.execute("SELECT balance FROM users WHERE id=?", (p['userId'],)).fetchone()
        if not user or user['balance'] < body.amount:
            raise HTTPException(400, "Insufficient balance")

        req_id = generate_id()
        conn.execute(
            """INSERT INTO payment_requests (id, user_id, amount, method, detail, status)
               VALUES (?, ?, ?, ?, ?, 'pending')""",
            (req_id, p['userId'], body.amount, body.method, body.detail)
        )
    return {"message": "Payout request submitted", "requestId": req_id}
...
async def list_payouts(request: Request):
    ...
    with get_db() as conn:
        if p['role'] == 'admin':
            rows = conn.execute("SELECT pr.*, u.username FROM payment_requests pr JOIN users u ON pr.user_id = u.id ORDER BY pr.created_at DESC").fetchall()
        else:
            rows = conn.execute("SELECT * FROM payment_requests WHERE user_id = ? ORDER BY created_at DESC", (p['userId'],)).fetchall()
        return {"data": [dict(r) for r in rows]}

class ReviewPayout(BaseModel):
    status: str # approved or rejected
    reason: Optional[str] = None

@router.post("/payouts/{req_id}/review")
async def review_payout(request: Request, req_id: str, body: ReviewPayout):
    p = _require(request)
    if p['role'] != 'admin': raise HTTPException(403, "Admin only")
    with get_db() as conn:
        req = conn.execute("SELECT * FROM payment_requests WHERE id = ?", (req_id,)).fetchone()
        if not req: raise HTTPException(404, "Request not found")
        if req['status'] != 'pending': raise HTTPException(400, "Request already reviewed")

        if body.status == "approved":
            # Deduct balance
            user = conn.execute("SELECT balance, username FROM users WHERE id = ?", (req['user_id'],)).fetchone()
            if user['balance'] < req['amount']:
                raise HTTPException(400, "User has insufficient balance now")
            ...
            conn.execute(
                """INSERT INTO transactions (id, user_id, username, tx_type, amount, balance_before, balance_after, note, created_by)
                   VALUES (?, ?, ?, 'payout', ?, ?, ?, ?, ?)""",
                (generate_id(), req['user_id'], user['username'], -req['amount'], user['balance'], new_balance, f"Payout approved: {req['method']}", p['userId'])
            )

        conn.execute(
            "UPDATE payment_requests SET status = ?, reviewed_by = ?, reason = ?, updated_at = datetime('now') WHERE id = ?",
            (body.status, p['userId'], body.reason, req_id)
        )
    return {"message": f"Payout {body.status}"}
