...
CAN_CREATE = {
    "admin":    {"admin", "manager", "reseller", "sub_reseller", "user"},
    "manager":  {"reseller", "sub_reseller", "user"},
    "reseller": {"sub_reseller", "user"},
    "sub_reseller": {"user"},
    "user":     set(),
}
...
@router.post("/registration-requests/{req_id}/review")
async def review_registration(request: Request, req_id: str, body: ReviewRegistration):
    p = _require_role(request, "admin", "manager")
    with get_db() as conn:
        ...
        if body.status == "approved":
            ...
            uid = generate_id()
            conn.execute(
                """INSERT INTO users (id, username, email, password, role, status, phone, country, parent_id)
                   VALUES (?, ?, ?, ?, ?, 'active', ?, ?, ?)""",
                (uid, req["username"], req["email"], req["password"], body.role, req["phone"], req["country"], p["userId"])
            )
            conn.execute("UPDATE registration_requests SET status = 'approved', reviewed_by = ?, updated_at = datetime('now') WHERE id = ?", (p["userId"], req_id))
        else:
            conn.execute("UPDATE registration_requests SET status = 'rejected', reviewed_by = ?, reason = ?, updated_at = datetime('now') WHERE id = ?", (p["userId"], body.reason, req_id))

    return {"message": f"Registration request {body.status}"}
