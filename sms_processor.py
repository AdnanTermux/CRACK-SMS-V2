...
def process_incoming_sms(payload=None, **kwargs) -> dict | list:
    """Process incoming SMS - auto-detects format"""
    if not payload and kwargs:
        # Handle direct keyword arguments (e.g. from SMPP)
        data = {
            'number': kwargs.get('to_number') or kwargs.get('number'),
            'message': kwargs.get('message'),
            'from': kwargs.get('from_sender') or kwargs.get('from'),
